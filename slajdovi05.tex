...
\begin{tabular}{p{5cm}p{7cm}}
  \begin{itemize}
    \item prve dve linije u \texttt{test}-u kreiraju dve lokalne promenljive, \texttt{amount} i \texttt{rate}
    \item one dobijaju vrednosti 1000 i 0.05
  \end{itemize}
&
\begin{verbatim}
def addInterest(balance, rate):
...
\begin{tabular}{p{5cm}p{7cm}}
  \begin{itemize}
    \item kontrola se dalje prenosi na funkciju \texttt{addInterest}
    \item formalni parametri \texttt{balance} i \texttt{rate} dobijaju vrednosti stvarnih parametara \texttt{amount} i \texttt{rate}
    \item iako \texttt{rate} postoji u obe funkcije, to su dve različite promenljive
  \end{itemize}
&
\begin{verbatim}
def addInterest(balance, rate):
...
\begin{tabular}{p{5cm}p{7cm}}
  \begin{itemize}
    \item dodela vrednosti za parametre \texttt{balance} i \texttt{rate} funkcije \texttt{addInterest} koristi vrednosti \myblue{stvarnih} parametara
  \end{itemize}
&
\begin{verbatim}
def addInterest(balance, rate):
...
\begin{tabular}{p{5cm}p{7cm}}
  \begin{itemize}
    \item izvršavanje prve linije u \texttt{addInterest} će kreirati novu promenljivu \texttt{newBalance}
    \item u drugoj liniji će \texttt{balance} primiti vrednost od \texttt{newBalance}
  \end{itemize}
&
\begin{verbatim}
def addInterest(balance, rate):
...
\begin{tabular}{p{5cm}p{7cm}}
  \begin{itemize}
    \item \texttt{balance} sada pokazuje na istu vrednost kao i \texttt{newBalance}
    \item ali to nema uticaja na \texttt{amount} u \texttt{test}-u
  \end{itemize}
&
\begin{verbatim}
def addInterest(balance, rate):
...
\begin{tabular}{p{5cm}p{7cm}}
  \begin{itemize}
    \item izvršavanje \texttt{addInterest} se završava i kontrola se vraća \texttt{test}-u
    \item lokalne promenljive iz \texttt{addInterest} više ne postoje, ali \texttt{amount} i \texttt{rate} u \texttt{test}-u pokazuju i dalje na stare vrednosti
  \end{itemize}
&
\begin{verbatim}
def addInterest(balance, rate):
...
\begin{tabular}{p{5cm}p{7cm}}
  \begin{itemize}
    \item prve dve linije u \texttt{test}-u inicijalizuju promenljive \texttt{amounts} i \texttt{rate}
    \item vrednost promenljive \texttt{amounts} je lista koja sadrži 4 int vrednosti
  \end{itemize}
&
\begin{verbatim}
def addInterest(balances, rate):
...
\begin{tabular}{p{5cm}p{7cm}}
  \begin{itemize}
    \item izvršava se \texttt{addInterest}
    \item petlja ide kroz listu 0, 1, 2, 3 i ažurira tu vrednost u \texttt{balances}
  \end{itemize}
&
\begin{verbatim}
def addInterest(balances, rate):
...
\begin{center}
  \includegraphics[width=10cm]{pic12}
\end{center}
\end{frame}
...
\begin{tabular}{p{5cm}p{7cm}}
  \begin{itemize}
    \item stare vrednosti na dijagramu nisu obrisane već su ostavljene da ,,vise`` da bismo naglasili da se nisu promenili brojevi u svojim kutijama, već su novi dodeljeni elementima postojeće liste
    \item stare vrednosti će iz memorije osloboditi \myblue{garbage collection} mehanizam
  \end{itemize}
&
\begin{verbatim}
def addInterest(balances, rate):
